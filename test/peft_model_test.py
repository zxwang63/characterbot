    ...
    **Attributes**:
    ...
    def from_pretrained(
        ...
        **kwargs: Any,
        ...
        def remove_lora_Bx(model, keys_to_remove=["lora_B1", "lora_B2", "lora_B3"]):
            """
            Recursively remove all specified lora_Bx layers from the model.
            
            Args:
                model (torch.nn.Module): The PyTorch model to modify.
                keys_to_remove (list): List of keys representing the lora_Bx layers to remove.
            
            Returns:
                None. The model is modified in place.
            """
            # Traverse all child modules
            for name, module in model.named_children():
                # If the module is a container (e.g., Sequential or ModuleList), recurse into it
                if isinstance(module, torch.nn.Module):
                    remove_lora_Bx(module, keys_to_remove)
                
                # If it's a module containing layers to remove, check for `lora_Bx`
                if hasattr(module, "lora_B1") or hasattr(module, "lora_B2") or hasattr(module, "lora_B3"):
                    for key in keys_to_remove:
                        if hasattr(module, key):
                            # Delete the lora_Bx module
                            delattr(module, key)
                            print(f"Removed {key} from {name}")

        # Example Usage:
        # Assuming `model` is your PeftModelForCausalLM instance
        remove_lora_Bx(model)
