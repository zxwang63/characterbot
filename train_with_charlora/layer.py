...
class LoraLayer(BaseTunerLayer):
    ...
    def update_layer(
        ...
        self.lora_B1[adapter_name] = nn.Linear(r, self.out_features, bias=False)
        self.lora_B2[adapter_name] = nn.Linear(r, self.out_features, bias=False)
        self.lora_B3[adapter_name] = nn.Linear(r, self.out_features, bias=False)
        if use_rslora:
...
class Linear(nn.Module, LoraLayer):
    ...
    def forward(self, x: torch.Tensor, datalabel: torch.Tensor = None, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            # Initialize the result from base layer output
            result = self.base_layer(x, *args, **kwargs)
            torch_result_dtype = result.dtype
            
            # If datalabel are not provided, retain original forward behavior
            if datalabel is None:
                ...
                # Conditional path based on datalabel (1 -> lora_B1, 2 -> lora_B2, 3 -> lora_B3)
                for active_adapter in self.active_adapters:
                    if active_adapter not in self.lora_A.keys():
                        continue
                    ...
                    # Handle different datalabel (1 -> lora_B1, 2 -> lora_B2, 3 -> lora_B3)
                    mask_B1 = datalabel == 1
                    mask_B2 = datalabel == 2
                    mask_B3 = datalabel == 3

                    # Process inputs through the appropriate LoRA branches
                    if mask_B1.any():
                        x_B1 = x[mask_B1]
                        lora_B1 = self.lora_B1[active_adapter]
                        if not self.use_dora[active_adapter]:
                            result[mask_B1] += lora_B1(lora_A(dropout(x_B1))) * scaling
                        else:
                            x_B1 = dropout(x_B1)
                            result[mask_B1] += self.lora_magnitude_vector[active_adapter](
                                x_B1,
                                lora_A=lora_A,
                                lora_B=lora_B1,
                                scaling=scaling,
                    ...
                    if mask_B2.any():
                        ...
                        if not self.use_dora[active_adapter]:
                            result[mask_B2] += lora_B2(lora_A(dropout(x_B2))) * scaling
                        else:
                            x_B2 = dropout(x_B2)
                            result[mask_B2] += self.lora_magnitude_vector[active_adapter](
                                x_B2,
                                lora_A=lora_A,
                                lora_B=lora_B2,
                                scaling=scaling,
                    ...
                    if mask_B3.any():
                        x_B3 = x[mask_B3]
                        lora_B3 = self.lora_B3[active_adapter]
                        if not self.use_dora[active_adapter]:
                            result[mask_B3] += lora_B3(lora_A(dropout(x_B3))) * scaling
                        else:
                            x_B3 = dropout(x_B3)
                            result[mask_B3] += self.lora_magnitude_vector[active_adapter](
                                x_B3,
                                lora_A=lora_A,
                                lora_B=lora_B3,
                                scaling=scaling,
