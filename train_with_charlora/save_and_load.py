...
def load_peft_weights(model_id: str, device: Optional[str] = None, **hf_hub_download_kwargs) -> dict:
    ...
    for key, value in adapters_weights.items():
        if ".lora_B." in key:
            ...
            updated_weights[key] = zero_value

            updated_weights[key.replace(".lora_B.", ".lora_B1.")] = copy.deepcopy(value)
            updated_weights[key.replace(".lora_B.", ".lora_B2.")] = copy.deepcopy(value)
            updated_weights[key.replace(".lora_B.", ".lora_B3.")] = copy.deepcopy(value)
